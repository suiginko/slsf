...
export type PlayerRole = 'RED_DESC' | 'RED_GUESS' | 'GREEN_DESC' | 'GREEN_GUESS' | 'HOST' | 'SPECTATOR';
...
export type OnlinePhase =
  ...
  | 'GUESSING_BUZZED'  // 触发抢答或防抢保护，20秒答题阶段
...
export interface OnlineCellData {
  ...
  owner: Team | null;
  revealed: boolean;
  word?: string;   // 仅当 revealed===true 或属于双方描述位/主持人推流时存在
  pinyin?: string; // 同上
}
...
export interface OnlineClueLog {
  id: string;
  timestamp: number;
  type: 'CLUE' | 'GUESS' | 'BUZZER_DEFEND' | 'BUZZER_HIJACK' | 'TIMEOUT' | 'SYSTEM' | 'HOST_ACTION';
  team: Team;
...
export interface OnlineRoomState {
  ...
  isProtected: boolean;      // 答题方是否已提前开启防抢保护
